<div class="clearfix wrapper">
   <div class="clearfix res_cat_card_header pb-2">
      ...
      // Get the sector terms for the cpt_resource post type
      ...
      $args = array(
         ...
         // Check if sector filter is set and not empty
         if (isset($_GET['sector-filter']) && !empty($_GET['sector-filter'])) {
            $sector_filter = sanitize_text_field($_GET['sector-filter']);
            if ($sector_filter !== '') {
         ...
         if ($query->have_posts()) {
            ...
            // if (isset($_GET['sector-filter']) && !empty($_GET['sector-filter'])) {
            // $sector_filter = sanitize_text_field($_GET['sector-filter']);
            // echo ' for <i><strong> ' . $sector_filter . '</strong></i>';
            // }
            //
            // echo '</p>'; 
            ...
            if (isset($_GET['sector-filter']) && !empty($_GET['sector-filter'])) {
               $sector_filter = sanitize_text_field($_GET['sector-filter']);
               $sector_term = get_term_by('slug', $sector_filter, 'sector'); // Get the term object
               if ($sector_term) {
                  echo ' for <i><strong> ' . $sector_term->name . '</strong></i>'; // Display the term name
               }
            }
            echo '</p>';
      ...
      <div class="fl width50 text-right">
         <form action="<?php echo esc_url( get_term_link( $term ) ); ?>" method="get" id="resource-filter-form">
             <label for="sector-filter"><?php esc_html_e( 'Filter by:', 'hitechdigital' ); ?></label>
             <select name="sector-filter" id="sector-filter">
               <!--option value=""><php esc_html_e( 'All', 'hitechdigital' ); ?></option-->
               <option value="" <?php selected(!isset($_GET['$sector-filter']) || $_GET['$sector-filter'] === ''); ?>><?php esc_html_e('All', 'hitechdigital'); ?></option>
                 <?php foreach ( $sectors as $sector ) : ?>
                     <option value="<?php echo esc_attr( $sector->slug ); ?>" <?php selected( isset( $_GET['sector-filter'] ) && $_GET['sector-filter'] === $sector->slug ); ?>><?php echo esc_html( $sector->name ); ?></option>
                 <?php endforeach; ?>
             </select>
...
jQuery(document).ready(function($) {
   ...
   $('#sector-filter').on('change', function() {
      ...
      currentURL.searchParams.set('sector-filter', selectedValue);
      ...
      $('.res_cat_card_wrap').load(currentURL.toString() + ' .res_cat_card_wrap', function() {
         ...
         // Update the message with the selected option text, sector, and term name
...
//      var currentURL = new URL(window.location.href);
//      currentURL.searchParams.set('sector-filter', selectedValue); // Update the URL parameter
//
//      // Load the content from the updated URL into the wrapper element
...
<?php
// AJAX callback to update the sector posts
//add_action('wp_ajax_update_sector_posts', 'update_sector_posts_callback');
//add_action('wp_ajax_nopriv_update_sector_posts', 'update_sector_posts_callback');
//function update_sector_posts_callback()
//{
//   $termID = intval($_POST['termID']);
//   $taxonomy = sanitize_text_field($_POST['taxonomy']);
//   $sectorFilter = sanitize_text_field($_POST['sectorFilter']);
//
...
<form action="<php echo esc_url( get_term_link( $term ) ); ?>" method="get">
    <label for="sector-filter"><php esc_html_e( 'Filter by sector:', 'text-domain' ); ?></label>
    <select name="sector-filter" id="sector-filter">
...
<form method="get">
    <label for="sector-filter"><php _e( 'Filter by sector:', 'text-domain' ); ?></label>
    <select name="sector-filter" id="sector-filter">
        <option value=""><php _e( 'All sectors', 'text-domain' ); ?></option>
        <php
        $terms = get_terms( array(
            'taxonomy' => 'sector',
            'hide_empty' => false,
        ...
        foreach ( $terms as $term ) {
            $option = '<option value="' . esc_attr( $term->slug ) . '"';
            if ( isset( $_GET['sector-filter'] ) && $_GET['sector-filter'] == $term->slug ) {
                $option .= ' selected';
            }
            $option .= '>' . esc_html( $term->name ) . '</option>';
            echo $option;
...
// Check for the sector filter parameter and add it to the query arguments if present
if ( isset( $_GET['sector-filter'] ) && ! empty( $_GET['sector-filter'] ) ) {
    $args['tax_query'][] = array(
        'taxonomy' => 'sector',
        'field' => 'slug',
